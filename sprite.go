...
func LoadSprites(arch *Archive, images Images) (Sprites, error) {
	...
	for file, data := range arch.Files {
		if path.Ext(file) == ".SPR" {
			...
			var sprite Sprite
			if err := binary.Read(reader, binary.LittleEndian, &numFrames); err != nil {
				return sprites, err
			}
			...
			sprite.Frames = make([]Frame, numFrames)
			sprite.Imgs = make([]*image.Paletted, numFrames)

			for i := uint16(0); i < numFrames; i++ {
